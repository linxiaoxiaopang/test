<template>
  ...
  >
      ...
      <el-row :gutter="10" class="mtb15">
        ...
        <el-col :span="8">
          <el-input
            clearable
            size="mini"
            placeholder="请输入自定义SKU搜索"
            suffix-icon="el-icon-search"
            v-model="query.custom_sku"
          >
          </el-input>
        </el-col>
        <el-col :span="8">
          ...
          <template #mergeSlot="{ scoped }">
            <div class="merge-slot-class">
              <el-image
                ...
                :preview-src-list="[figure(scoped.prod_show_images)]"
                :z-index="3000"
                fit="contain"
              ></el-image>
              <div>
                <div>产品名称: {{ scoped.chinese_name || '暂无名字' }}</div>
                <div>
                  产品分类: {{ getLabel('proCategory', scoped.category) }}
                </div>
              </div>
            </div>
          </template>
    ...
    <template #footer>
      ...
      >
        <template #layout="{ scoped: { totalPage } }">
          <span style="text-align: center">共 {{ totalPage }} 页</span>
          <color-text-btn
            type="primary"
            style="text-align: center; margin: 0 15px"
            >当前页{{ domData.length }} 条SKU</color-text-btn
          >
        </template>
      </PaginationBar>
    </template>
...
import { PICSIZE, WAIT_DISPATCH, WAIT_GENERATE_WAYBILL } from '@/utils/constant'
import { relationRelationSku as associate } from '@/api/product/knifeApi'
import { orderUpdate } from '@/api/order'
import { relevanceUpdate as edit } from '@/api/order'
import colorTextBtn from '@/components/base/colorTextBtn.vue'
import throttle from 'lodash/throttle'
import { validatenull } from '@/components/avue/utils/validate'
import { mapGetters } from 'vuex'
import { delDicEmptyChildren, getLabel } from '@/components/avue/utils/util'
...
export default {
  ...
  computed: {
    ...
    proCategoryDic() {
      return delDicEmptyChildren(this.proCategory)
    },
    domData({ query: { sys_sku } }) {
      const data = cloneDeep(this.data)
      const tmpArr = []
      data.map((item) => {
        try {
          const { products = [], chinese_name, category, id: tId } = item
          products.map((pItem) => {
            const { spec_prods, id: proId, ...rest } = pItem
            spec_prods.map((item) => {
              if (
                validatenull(sys_sku) ||
                [sys_sku.toLowerCase(), sys_sku.toUpperCase()].some((sku) =>
                  item.SKU.includes(sku)
                )
              ) {
                rest.custom_sku = item.custom_sku || '暂无'
                tmpArr.push(
                  Object.assign(
                    item,
                    { proId, chinese_name, category, tId },
                    rest
                  )
                )
              }
  ...
  methods: {
    ...
    doRelation(row) {
      console.log(row)
      const { SKU, id, custom_sku } = row
      this.$alert(
        `确定外部 SKU:<span style="color: #C03639;">${
          this.info.productExternalSkuCode
        }</span>与自定义SKU：<span style="color: #C03639;">${
          custom_sku || '暂无'
        }</span>匹配关联吗？再次导入 同一SKU时不会再做匹配`,
        '确认',
        {
    ...
    async edit(productInternalSkuCode) {
      ...
      const { code, detail } = await edit({
        productInternalSkuCode,
        id: this.info.id,
        productCount: this.info.productCount,
        productExternalSkuCode: this.info.productExternalSkuCode
      })
      if ($SUC({ code })) {
        this.info.productInternalSkuCode = productInternalSkuCode
        console.log('detail yzb', detail)
        this.$emit('edited', this.info)
        if (false) {
          ...
          if (this.sup_this) {
            let { detail } = (await this.sup_this.init()) || {}
            this.$store.dispatch('GetOrderCount')
