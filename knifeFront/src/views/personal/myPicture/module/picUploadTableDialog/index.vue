<template>
  <BaseDialog class="picUploadTableDialogComponent" :width="$attrs.width || '80%'" v-bind="$attrs" v-on="$listeners">
    <template>
      <CommonTable
        ref="table"
        height="auto"
      ...
      >
        <template #empty>
          <el-button @click="uploadMorePic" class="empty-btn" type="text" icon="el-icon-plus">添加图片</el-button>
        </template>
        <template #pathSlot="{ scoped: row, prop }">
          <CacheImg ref="img" style="width: 80px; height: 80px" :raw="row.raw" fit="contain" />
        </template>
        <template #titleHeaderSlot>
          <div class="title">名称</div>
        </template>
        <template #existSlot="{ scoped }">
          <el-tag type="warning" v-if="scoped[scoped.prop]"> 已存在或后台已禁用，无需重复上传。 </el-tag>
          <el-tag type="warning" v-else-if="scoped.$existSameTitle"> 存在标题相同图片。 </el-tag>
        ...
        <template #category_nameHeaderSlot>
          ...
          <div class="sub-title">
            <CateHead :sup_this="cur_this" @updateSelectedData="updateSelectedData" />
          </div>
        </template>
        <template #category_nameSlot="{ scoped }">
          ...
          <div class="sub-title">
            <Cate
              :data="scoped"
              :sup_this="cur_this"
              @updateSelectedData="updateSelectedData($event, data[scoped.index] || {})"
            />
          </div>
        </template>
        <slot>
          <el-table-column label="操作" align="center" width="80px">
            <template slot-scope="scoped">
              <LoadingBtn type="text" @click="del(scoped.$index)" size="mini">删除</LoadingBtn>
            </template>
          </el-table-column>
        </slot>
      </CommonTable>
    ...
    <template #footer>
      <el-button :disabled="tableLoading" type="primary" @click="uploadHandler"> 上传 </el-button>
...
import { check } from '@/api/imageApi'
...
export default {
  ...
  methods: {
    ...
    validateExistSameData() {
      let { data } = this
      let sameTitles = []
      data.forEach(({ title }, index) => {
        if (data.findIndex(({ title: innerTitle }) => innerTitle === title) !== index) sameTitles.push(title)
      })
      data.forEach((item) => {
        this.$set(item, '$existSameTitle', sameTitles.includes(item.title))
      })
    },
    ...
    async uploadHandler() {
      let { data } = this
      const uploadData = []
      const existSameTitles = []
      data.forEach((item) => {
        if (item.exist) uploadData.push(item)
        if (item.$existSameTitle) existSameTitles.push(item)
      })
      if (data.length === 0) {
      ...
      if (existSameTitles.length) {
        this.$message.warning('存在标题相同图片')
        return
      }
      data.map((form) => {
        this.doAdd(form)
    ...
    checkTag(tags) {
      ...
      if (errorStr.length > 0) {
        this.$message(`标签只能是包含大小写字母，中文，_的非空字符串, ${errorStr.join('，')}标签存在非法字符！！`)
        return false
      }
      return true
    }
