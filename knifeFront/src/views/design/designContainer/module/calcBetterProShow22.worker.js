  ...
  for (let j = 0; j < show_image.rows; ++j) {
    for (let i = 0; i < show_image.cols; ++i) {
      ...
      if (alpha != 0) {
        ...
        show_image.data[j * show_image.cols * nchannels + i * nchannels + 0] = getUchar(
          alpha_f * tmp_eff_color[0] + (1.0 - alpha_f) * tmp_show_color[0])
        show_image.data[j * show_image.cols * nchannels + i * nchannels + 1] = getUchar(
          alpha_f * tmp_eff_color[1] + (1.0 - alpha_f) * tmp_show_color[1])
        show_image.data[j * show_image.cols * nchannels + i * nchannels + 2] = getUchar(
