onmessage = async function (evt) {
  let { show_img, shadow_img, data, showMarks } = evt.data
...
function hasMaskFlag(front_mask, x, y) {
  ...
  try {
    return (
      channels[0] != 0 && channels[1] && channels[2] != 0 && channels[3] != 0
    )
  } catch (err) {
    return false
  }
