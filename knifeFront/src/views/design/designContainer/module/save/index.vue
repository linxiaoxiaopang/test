...
export default {
  ...
  methods: {
      ...
      const params = fabricList.map(async (instance, instanceIndex) => {
        ...
        objects.map(async (obj, index) => {
          ...
          if (isGroup) {
            ...
            const { scaleX, scaleY, id: pId } = fObj
            ...
            os.map((oLayer, i) => {
              ...
              h2 = h2 * scaleY
              if (fObj.angle) {
                const large = Math.sqrt(w2 * w2 + h2 * h2)
                w2 = large
                h2 = large
              }
        ...
        //将数据转换成平铺数据
        dataRes = this.formatImgsToTile(dataRes)
        return {
    ...
    formatImgsToTile(data) {
      const tmpObj = {}
      data.map((item) => {
        const { pId, originId, flipX, flipY, img_id, offset_x, offset_y, rotation, scaleX, scaleY } = item
        const id = pId || originId
        if (!tmpObj[id]) {
          tmpObj[id] = {
            flipX,
            flipY,
        ...
        tmpObj[id].list.push({ offset_x, offset_y })
      })
    ...
    async conversionLayerObject(layerObj, instanceIndex) {
      ...
      let { scaleX, scaleY, id, flipX, flipY, width: picWidth, height: picHeight, pId, id: originId } = layerObj
