...
import { SECURITY, MAKER } from '@/utils/constant'
...
export default {
  ...
  computed: {
    canvasBgPic() {
      ...
      return tmpArr[0] && tmpArr[0].img_url
      ...
      try {
        this.$nextTick(async () => {
          ...
          const imgP = new Promise((resolve) => {
            ...
            img.src = this.canvasBgPic
          })

          const markImgP = new Promise((resolve) => {
            markImg.setAttribute('crossOrigin', 'Anonymous')
            markImg.onload = markImg.onerror = () => {
              resolve(true)
            }
            markImg.src = this.makerPic
          })
          await Promise.all([imgP, markImgP])
          const {
            w: markW,
            h: markH,
            dirW: markDirW,
          ...
          ctx.drawImage(markImg, markDirW, markDirH, markW, markH)

          const imgData = ctx.getImageData(markDirW, markDirH, markW, markH)
          ...
          ctx.putImageData(imgData, markDirW, markDirH)
          const {
            w: imgW,
            h: imgH,
            dirW: imgDirW,
          ...
          ctx.drawImage(img, imgDirW, imgDirH, imgW, imgH)
