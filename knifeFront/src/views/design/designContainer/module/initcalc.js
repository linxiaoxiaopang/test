...
// show_image_mapxy,
// kl_image
import { DESIGN_SHOW_LONG_NUM } from '@/utils/constant'
...
export function initShowImageMapxy(info) {
  ...
  console.log(show_image.naturalWidth, show_image.naturalHeight)
  console.log(show_img_rows, show_img_cols)
...
function initJsonData(root) {
  ...
  try {
    ...
    showMap.show_mask_height = parseInt(root['show_mask_height'])
    showMap.show_mask_left = parseInt(root['show_mask_left'])
    showMap.show_mask_top = parseInt(root['show_mask_top'])
    showMap.show_mask_width = parseInt(root['show_mask_width'])
    for (let i = 0; i < au_size; ++i) {
      const tmapU = {}
      tmapU.id1 = parseInt(root['Au'][i][0])
      tmapU.id2 = parseInt(root['Au'][i][1])

      tmapU.id3 = parseInt(root['Au'][i][2])
      tmapU['A'] = []
      for (let j = 0; j < 6; j++) {
      ...
      // Ad 数据解析
      const tmapD = {}
      tmapD.id1 = parseInt(root['Ad'][i][0])
      tmapD.id2 = parseInt(root['Ad'][i][1])
      tmapD.id3 = parseInt(root['Ad'][i][2])
      tmapD['A'] = []
      for (let j = 0; j < 6; ++j) {
    ...
    for (let i = 0; i < model_size; ++i) {
      const tmp = {}
      tmp.x = parseInt(root['model_corners'][i][0])
      tmp.y = parseInt(root['model_corners'][i][1])
      modelPts.push(tmp)
    }
  } catch (err) { }
  return {
...
function maskAlphaValue(front_mask, x, y) {
  ...
  if (nChannel == 4) {
    return parseInt(front_mask.data[4 * y * front_mask.cols + 4 * x + 3])
  } else if (nChannel == 1) {
