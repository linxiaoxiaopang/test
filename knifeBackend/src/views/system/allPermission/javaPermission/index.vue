    ...
    >
      <template #rightOperationBtn>
        <LoadingBtn
          ...
          v-p="['rbac:permissions:bulk_create']"
          type="primary"
          size="mini"
          @click="bulkDoAdd"
        >
      ...
      <slot>
        <el-table-column label="操作" width="300px" align="center">
          <template slot-scope="scope">
            ...
            />
            <AddChilds
              v-p="['rbac:permissions:add_childs']"
              :data="scope.row"
              :permissions="permissionChildSelectList"
              :sup_this="sup_this"
              v-if="!scope.row.method"
...
export default {
  ...
  data() {
    return {
      value: localStorage.getItem("props"),
      columns: [
        {
          text: "名称",
          value: "name",
      ...
      permissionGroupList: [],
      url: localStorage.getItem("props")
        ? this.formatUrl(localStorage.getItem("props"))
        : "/api/common/perms_manage",
      size: 1000,
    };
  },
  ...
  methods: {
    ...
    changeValue() {
      ...
      localStorage.setItem("props", this.value);
      this.url = this.formatUrl(this.value);
      this.init();
    },
    formatUrl(value) {
      const props = value.split(",");
      let url = "/api/common/perms_manage";
