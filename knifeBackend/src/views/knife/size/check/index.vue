...
import { createRandomNum, convertCurrency, uniqueArray, debounce, getPicName, getValueFromObj } from '@/utils'
...
export default {
  ...
  created() {
    ...
    getReferenceSize(this.$route.query.primId).then(
      ({
        ...
        if (this.rotateBaseSize.base_kl_groups && this.rotateBaseSize.base_kl_groups.length) {
  ...
  methods: {
    ...
    async submitRotate() {
      try {
        const { rotateData } = this.$refs.rotateProductComponent
        const { base_kl_groups: list } = this.baseSize
        const existPicData = []
        list.forEach((rotate, index) => {
          existPicData.push({
            quick_design_rotate: rotateData[index],
            id: list[index] && list[index].id
          })
        })
    ...
    createPsdListData() {
      ...
      try {
        ...
        Promise.all(p)
          ...
          .catch((err) => {
            this.canSave = false
            this.$message.warning('psd解析失败，请重新上传')
            setTimeout(() => {
              this.$router.back()
            }, 1000)
          })
    ...
    validPsdGroupsLen() {
      ...
      if (
        base_kl_groups &&
        psdListData.length === psdList.length &&
