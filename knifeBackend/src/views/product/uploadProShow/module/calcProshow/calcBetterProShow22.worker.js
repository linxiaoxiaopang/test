...
onmessage = async function (evt) {
  console.log('join')
  let { white_shows, shadow_show, allData, show_image_mapxy } = evt.data
  const pArr = allData.map((data, index) => {
    const white_show = white_shows[index]
    data.map((item) => {
    ...
    return blobToDataURL(blob).then((imageURL) => {
      return imageURL
    })
  })
  const allImgData = await Promise.all(pArr)
  postMessage(allImgData)
}
