  ...
  <div class="referenceCanvas">
    <div class="doc">
      ...
      <draggable class="list" v-model="data" v-bind="dragOption" @change="dragChange($event)">
        <div
          :key="index"
          :class="['item', 'drag-item', isSet({ name, base_group_cover }) && 'is-set']"
          v-for="({ img_url, show_rotate, name, base_group_cover, text }, index) in forms"
        >
...
export default {
  ...
  computed: {
    ...
    isSet() {
      return ({ name, base_group_cover }) => {
        try {
          return !!(name && base_group_cover.length)
        } catch (err) {
          return false
        }
      }
