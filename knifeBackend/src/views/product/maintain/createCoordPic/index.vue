    ...
    <div class="content-wrapper">
      ...
      <div class="header">
        ...
        <Popover @sureHandler="clipCoordPic($event)">
          ...
          <template #reference="{ scope: loading }">
            <el-button
              ...
              v-p="['prim_prod:change_coord_img:create']"
              >更新坐标图</el-button
            >
          </template>
        </Popover>
        <el-button
          ...
          class="label"
          @click="clipCoordPic"
          v-p="['prim_prod:upload_coord_img:create']"
          >生成坐标图</el-button
        >
      </div>
    </div>
...
import { DPI_CM_FROM_IN } from '@/utils/constant'
...
export default {
  ...
  computed: {
    isLack() {
      const isLack = !this.structs.every(({ prim_show_groups }) => {
        return prim_show_groups[0] && prim_show_groups[0].coord_image
      })
      return isLack
    }
  },
    ...
    async clipCoordPic($el) {
      ...
      try {
        ...
        const params = {
          ...
          gridsClipPics,
          structIds: structId && structId.split(','),
          sizeId,
          protoId,
