<template>
  ...
  <section class="app-container app-flex-col-container protoCategoryPage">
    <EHeader :query="query" :sup_this="sup_this" v-p="['image:image_category:create']"/>
        ...
        <el-table-column align="center" label="操作" width="280">
          <template slot-scope="{ row, $index }">
            <template v-if="$index !== 0">
              <AddChildCate :sup_this="sup_this" :data="row" v-p="['image:image_category:create']"></AddChildCate>
              <EditCate :sup_this="sup_this" :data="row" v-p="['image:image_category:partial_update']" />
              <DelCate :sup_this="sup_this" :data="row" v-p="['image:image_category:delete']" />
            </template>
