...
export default function () {
  return {
    ...
    methods: {
      ...
      getDicData(ele) {
        // 处理字典
        if (validatenull(ele.dicData) || ele.dicData === ele.prop) {// ele.dicData为空，或者ele.dicData等于ele.prop（为了重新请求字典）
          ele.dicData = null
          if (!validatenull(ele.dicUrl)) {// ele.dicUrl不为空
            if (ele.dicUrl.indexOf('/admin/dict/type/') > -1) {// ele.dicUrl包含/admin/dict/type/时，将字典名赋值给ele.dicData
              ele.dicData = ele.dicUrl.replace('/admin/dict/type/', '')
            } else if (ele.dicUrl.indexOf('{{key}}') === -1) {// 非多级联动子节点
