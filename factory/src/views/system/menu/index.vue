<template>
  <div>
    <menuTreeTable :data="addDomNewMenus" operationTitle="待新建菜单列表" :tableLoading="tableLoading">
      <template #rightOperationBtn>
        <LoadingBtn v-if="!data.length" type="primary" size="mini" @click="bulkDoAdd"> 新建全部 </LoadingBtn>
      </template>
      <slot>
        <el-table-column label="操作" width="80px" align="center" v-if="data.length">
          <template slot-scope="{ row }">
            <add :menus="menus" :delMenus="delDomMenus" :data="row" :sup_this="sup_this" />
          </template>
        </el-table-column>
      </slot>
    </menuTreeTable>

    <menuTreeTable :data="delDomMenus" operationTitle="待删除菜单列表" :tableLoading="tableLoading">
      <slot
    ...
    <menuTreeTable :data="domData" operationTitle="列表展示" :tableLoading="tableLoading">
      <slot>
        <el-table-column label="操作" width="80px" align="center">
          <template slot-scope="scope">
...
import { MENU_TYPE } from '@/utils/constant' // '后台端'

const MENUS_NAME = '工厂端'
export default {
  components: {
    menuTreeTable,
    CommonTreeTable,
    ...
    data: {
      handler() {
        this.$nextTick(() => {
          ...
          this.delMenus = treeData(bothDifferent(shakingChildren(this.data), shakingChildren(superRouter)), null)
        })
      },
      deep: true
    }
  ...
  methods: {
    ...
    async bulkDoAdd(menu) {
      ...
      if (!menu) {
        ...
        const { path, component, icon, id, pid, is_frame, is_show = true, name, sort } = menu
        params = [
          {
            path,
            component,
